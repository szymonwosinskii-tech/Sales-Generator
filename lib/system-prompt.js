...
- Rental business: most products have rent_ok true, sale_ok false. The RENTAL PERIOD LIVES ON THE
  ORDER HEADER (sale.order), not per-line: is_rental_order (boolean, must be true for the rental
  period fields to even appear/apply), rental_start_date (datetime), rental_return_date (datetime).
  ALWAYS set all three of these on every sale.order you create that has rented products - if you
  omit is_rental_order or the two dates, Odoo will NOT show the rental-period UI at all and the
  order will look like a broken plain quotation. If the customer's message hasn't given you both a
  drop-off and a return date/time, ASK before creating anything - don't guess and don't create the
  order without them.
